...
import { Flex, Modal, Badge, Button } from "@mantine/core";
...
export const CardWithModal: React.FC<CardProps> = ({ productId }) => {
  ...
  return (
    <Flex className="ProductCard" direction="column">
      ...
      >
        ...
        {quantity === 0 ? (
          <Button variant="filled" fullWidth rightSection={<IconPlus size={14} />} onClick={handleAddProduct}>
            <span className="ProductCard--button__add">Thêm vào giỏ</span>
            <span className="ProductCard--button__addIcon">+</span>
          </Button>
        ) : (
          <button className="ProductCard--button">
            <span
              className="ProductCard--button__icon leftIcon"
            ...
            <span
              className="ProductCard--button__icon rightIcon"
